...
class Program
{
    ...
    static void Main()
    {
        ...
        bool IsCatchingUp()
        {
            ...
            int targetDealerCards = (currentState == GameState.Playing) ? 1 : game.Dealer.hand.Count;
            if (dealerVisualCards < targetDealerCards)
            {
                return true;
            }
            
            if (playerVisualCards.ContainsKey(game.player) && playerVisualCards[game.player] < game.player.hand.Count)
            {
                return true;
            }
            ...
            foreach (var bot in game.ActiveBots)
            {
                if (playerVisualCards.ContainsKey(bot) && playerVisualCards[bot] < bot.hand.Count)
                {
                    return true;
                }
            }
        ...
        InitVisuals();
        Rectangle hitButton = new Rectangle(460, 1040, 200, 80);
        Rectangle standButton = new Rectangle(700, 1040, 200, 80);
        Rectangle doubleButton = new Rectangle(940, 1040, 200, 80);
        ...
        while (!Raylib.WindowShouldClose() && keepPlaying)
        {
            if (currentAnim.Active)
            {
                ...
                if (currentAnim.Progress >= 1f)
                {
                    ...
                    currentAnim.Active = false;
                    if (currentAnim.Target is Dealer)
                    {
                        dealerVisualCards++;
                    }
                    else if (currentAnim.Target is Player p)
                    {
                        playerVisualCards[p]++;
                    }
                }
            ...
            if (Raylib.IsMouseButtonPressed(MouseButton.Left))
            {
                ...
                {
                    if (Raylib.CheckCollisionPointRec(mousePos, resumeBtn))
                    {
                        currentState = previousState;
                    }
                    else if (Raylib.CheckCollisionPointRec(mousePos, rulesBtn))
                    {
                        currentState = GameState.Rules;
                    }
                    else if (Raylib.CheckCollisionPointRec(mousePos, quitBtn))
                    {
                        keepPlaying = false;
                    }
                }
                ...
                else if (!IsCatchingUp())
                {
                    ...
                    else if (currentState == GameState.Betting)
                    {
                        if (Raylib.CheckCollisionPointRec(mousePos, betMinusBtn)) 
                        {
                            ...
                            if (currentBet > game.player.Money)
                            {
                    ...
                    else if (currentState == GameState.Playing)
                    {
                        ...
                        else if (Raylib.CheckCollisionPointRec(mousePos, doubleButton) && game.player.hand.Count == 2 && game.player.Money >= game.player.Bet)
                        {
            ...
            if (currentState != GameState.Betting && currentState != GameState.Rules && currentState != GameState.TotalLoss)
            {
                ...
                if (currentState == GameState.Playing && dealerVisualCards == 1 && game.Dealer.hand.Count >= 2)
                {
                    Rectangle hiddenDest = new Rectangle(currentCardX, dYOffset, destCardWidth, destCardHeight);
                    Raylib.DrawRectangleRec(hiddenDest, Color.DarkBlue);
                    Raylib.DrawRectangleLinesEx(hiddenDest, 2, Color.White);
                }
            }
            ...
            if (currentState != GameState.Betting && currentState != GameState.Rules && currentState != GameState.TotalLoss)
            {
                ...
                if (currentState == GameState.GameOver && roundResults.ContainsKey(game.player) && !IsCatchingUp())
                {
                    ...
                    int textW = (int)Raylib.MeasureTextEx(uaFont, res, 36, 2).X; 
                    Raylib.DrawRectangle(playerX - 5, textY - 5, textW + 10, 36 + 10, new Color(0, 0, 0, 150)); 
                    DrawUA(res, playerX, textY, 36, Color.Yellow);
                }
            }
            ...
            for (int i = 0; i < game.ActiveBots.Count; i++)
            {
                ...
                if (currentState != GameState.Betting && currentState != GameState.Rules && currentState != GameState.TotalLoss)
                {
                    ...
                    if (currentState == GameState.GameOver && roundResults.ContainsKey(bot) && !IsCatchingUp())
                    {
                        ...
                        int textW = (int)Raylib.MeasureTextEx(uaFont, res, 32, 2).X; 
                        Raylib.DrawRectangle(bX - 5, textY - 5, textW + 10, 32 + 10, new Color(0, 0, 0, 150)); 
                        DrawUA(res, bX, textY, 32, Color.Yellow);
                    }
                }
            }
            ...
            if (currentState == GameState.Betting)
            {
                ...
                Raylib.DrawRectangleRec(betMinusBtn, Color.LightGray); 
                DrawUA("-", (int)betMinusBtn.X + 30, (int)betMinusBtn.Y + 20, 40, Color.Black);
                Raylib.DrawRectangleRec(betPlusBtn, Color.LightGray); 
                DrawUA("+", (int)betPlusBtn.X + 24, (int)betPlusBtn.Y + 20, 40, Color.Black);
                Raylib.DrawRectangleRec(dealBtn, Color.Gold); 
                DrawUA($"ГРАТИ: {currentBet}$", (int)dealBtn.X + 30, (int)dealBtn.Y + 20, 40, Color.Black);
                Raylib.DrawRectangleRec(betMaxBtn, Color.Red); 
                DrawUA("ВА-БАНК", (int)betMaxBtn.X + 70, (int)betMaxBtn.Y + 10, 40, Color.White);
            }
            else if (currentState == GameState.Playing && !IsCatchingUp())
            {
                Raylib.DrawRectangleRec(hitButton, Color.LightGray); 
                DrawUA("ЩЕ", (int)hitButton.X + 64, (int)hitButton.Y + 20, 40, Color.Black);
                Raylib.DrawRectangleRec(standButton, Color.LightGray); 
                DrawUA("ДОСИТЬ", (int)standButton.X + 30, (int)standButton.Y + 20, 40, Color.Black);
                
                if (game.player.hand.Count == 2 && game.player.Money >= game.player.Bet)
                {
                    Raylib.DrawRectangleRec(doubleButton, Color.Orange); 
                    DrawUA("ПОДВОЇТИ", (int)doubleButton.X + 8, (int)doubleButton.Y + 25, 36, Color.Black);
                }
            }
            else if (currentState == GameState.GameOver && !IsCatchingUp())
            {
                Raylib.DrawRectangleRec(playAgainBtn, Color.Blue); 
                DrawUA("ЩЕ РАУНД", (int)playAgainBtn.X + 80, (int)playAgainBtn.Y + 30, 40, Color.White);
            }
            ...
            if (currentState == GameState.TotalLoss && !IsCatchingUp())
            {
                ...
                DrawUA("НОВА ГРА", (int)restartBtn.X + 160, (int)restartBtn.Y + 30, 40, Color.White);
                Raylib.DrawRectangleRec(quitLossBtn, Color.Maroon); 
                DrawUA("ВИЙТИ", (int)quitLossBtn.X + 185, (int)quitLossBtn.Y + 30, 40, Color.White);
            }
            else if (currentState == GameState.Paused || currentState == GameState.Rules)
            {
                ...
                if (currentState == GameState.Paused)
                {
                    ...
                    DrawUA("ПРОДОВЖИТИ", (int)resumeBtn.X + 70, (int)resumeBtn.Y + 30, 40, Color.Black);
                    Raylib.DrawRectangleRec(rulesBtn, Color.LightGray); 
                    DrawUA("ПРАВИЛА", (int)rulesBtn.X + 115, (int)rulesBtn.Y + 30, 40, Color.Black);
                    Raylib.DrawRectangleRec(quitBtn, Color.Red); 
                    DrawUA("ВИЙТИ З ГРИ", (int)quitBtn.X + 85, (int)quitBtn.Y + 30, 40, Color.White);
                }
                else if (currentState == GameState.Rules)
                {
                     ...
                     DrawUA("6. 'Подвоїти' - подвоїти ставку і взяти рівно 1 карту (при 2 картах).", 160, 800, 40, Color.White);
                     
                     Raylib.DrawRectangleRec(backBtn, Color.LightGray);
                     
                     if (isFirstTimeRules)
                     {
                         DrawUA("ЗРОЗУМІЛО", (int)backBtn.X + 100, (int)backBtn.Y + 30, 40, Color.Black);
                     }
                     else
                     {
                         DrawUA("НАЗАД", (int)backBtn.X + 140, (int)backBtn.Y + 30, 40, Color.Black);
                     }
                }
            }
            else if (!IsCatchingUp())
            {
                ...
                DrawUA("||", (int)pauseBtn.X + 30, (int)pauseBtn.Y + 20, 40, Color.White);
            }

            Raylib.EndDrawing();
        }
        
        Raylib.UnloadFont(uaFont); 
