...
namespace Game
{
    public class Game
    {
        ...
        public Game()
        {
            ...
            this.ActiveBots = new List<Player> 
            { 
                new Bot_1(), 
                new Bot_2(), 
                new Bot_3() 
        ...
        public void PlayerDoubleDown()
        {
            if (this.player.Money >= this.player.Bet)
            {
                ...
                this.player.makeBet(this.player.Bet * 2);
                OnMessageSent?.Invoke(this.player, $"Ставку подвоєно! Нова ставка: {this.player.Bet}", MessageType.Info);
                Card drawnCard = this.Dealer.deck.DrawCard();
                this.player.getCard(drawnCard);
                OnMessageSent?.Invoke(this.player, $"Ви витягнули: {drawnCard.ToString()} | Ваші очки: {this.player.Points}", MessageType.TurnAction);
                if (this.player.Points > 21)
        ...
        public void EndGame()
        {
            ...
            {
                bool isHuman = (p == this.player);
                string botName = p.GetType().Name;
                
                if (p.Points > 21)
                {
                    string msg = isHuman ? $"Ви перебрали ({p.Points})!" : $"{botName}: Перебір ({p.Points})!";
                    OnMessageSent?.Invoke(p, msg, MessageType.Result);
                }
                else if (p.Points == 21)
                {
                    if (p.hand.Count == 2)
                    {
                        string msg = isHuman ? "У вас БЛЕКДЖЕК!" : $"{botName}: БЛЕКДЖЕК!";
                        OnMessageSent?.Invoke(p, msg, MessageType.Result);
                        p.Money += p.Bet * 3;
                    }
                    else
                    {
                        string msg = isHuman ? "Ви виграли (21)!" : $"{botName} виграв (21)!";
                        OnMessageSent?.Invoke(p, msg, MessageType.Result);
                        p.Money += p.Bet * 2;
                    }
                }
                ...
                {
                    string msg = isHuman ? "Ви виграли!" : $"{botName} виграв!";
                    OnMessageSent?.Invoke(p, msg, MessageType.Result);
                    p.Money += p.Bet * 2;
                }
                else if (p.Points == dealerTotal)
                {
                    string msg = isHuman ? "У вас нічия." : $"{botName}: Нічия.";
                    OnMessageSent?.Invoke(p, msg, MessageType.Result);
                    p.Money += p.Bet;
                }
                else
                {
                    string msg = isHuman ? "Ви програли." : $"{botName} програв.";
                    OnMessageSent?.Invoke(p, msg, MessageType.Result);
                }
            }
